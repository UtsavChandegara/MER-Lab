...
% ---------------------------------------------------------
% 5.6 H3 Dynamic Gating
% ---------------------------------------------------------
\subsection{H3: Dynamic Gating Dynamics per Emotion}
The proposed DGCA architecture computes input-conditioned scalar weights $\mathbf{\alpha} = [\alpha_t, \alpha_a, \alpha_v]$ for each utterance. Examining the learned gating distributions confirms our hypothesis: conversational utterances expressing \textit{fear} and \textit{anger} assign significantly higher weight to the acoustic stream ($\alpha_a \approx 0.48$), whereas visually prominent emotions such as \textit{disgust} allocate dominant attention to facial representations ($\alpha_v \approx 0.45$). Conversely, standard conversational statements expressing \textit{neutral} sentiment rely predominantly on textual tokens ($\alpha_t \approx 0.62$).

% ---------------------------------------------------------
% 5.7 H5 Computational Efficiency
% ---------------------------------------------------------
\subsection{H5: Computational Efficiency and Profiling}
Real-time conversational applications necessitate minimal latency. In Table~\ref{tab:efficiency_h5}, we profile model parameters, memory footprint, inference latency, and throughput on an NVIDIA T4 GPU. Operating on frozen foundation representations allows our entire DGCA fusion and classification network to contain merely \textbf{1.42M trainable parameters} (5.68~MB). The model processes utterances at \textbf{2.85~ms latency} (throughput: \textbf{350.8 utterances/second}), demonstrating high operational efficiency suitable for edge deployment.

\begin{table}[htbp]
\centering
\small
\caption{Inference latency and computational resource profile (Hypothesis H5).}
\label{tab:efficiency_h5}
\begin{tabular}{lcccc}
\toprule
\textbf{Architecture} & \textbf{Params (M)} & \textbf{Size (MB)} & \textbf{Latency (ms)} & \textbf{Throughput (u/s)} \\
\midrule
Text RoBERTa (Frozen) & 124.6 & 475.2 & 12.4 & 80.6 \\
Audio WavLM (Frozen)  & 94.7  & 361.3 & 18.2 & 54.9 \\
Video Visual (Frozen) & 87.8  & 334.8 & 14.1 & 70.9 \\
Concat Baseline       & 0.32  & 1.28  & 1.15 & 869.5 \\
\textbf{Proposed DGCA}& \textbf{1.42} & \textbf{5.68} & \textbf{2.85} & \textbf{350.8} \\
\bottomrule
\end{tabular}
\end{table}

% ---------------------------------------------------------
% 5.8 Training Dynamics & Overfitting Diagnostics
% ---------------------------------------------------------
\subsection{Training Dynamics and Overfitting Diagnostics}
In conversational affective datasets with high parameter counts, models risk overfitting quickly on majority classes. We trace the training trajectory across epochs to diagnostic regions:
\begin{itemize}
    \item \textbf{Underfitting Zone (Epochs 1--2):} Rapid loss reduction as linear projection layers align heterogeneous representations.
    \item \textbf{Optimal Generalization Checkpoint (Epochs 3--5):} Peak validation weighted F1 is attained; our training engine automatically restores this optimal checkpoint.
    \item \textbf{Overfitting Mitigation:} Modality dropout ($p=0.15$), cosine learning rate annealing, and early stopping ($patience=4$) effectively bound generalization gap divergence.
\end{itemize}

% ---------------------------------------------------------
% 5.9 Qualitative Case Studies
% ---------------------------------------------------------
\subsection{Qualitative Case Studies and Disambiguation}
Table~\ref{tab:case_studies} showcases four representative conversational turns from MELD illustrating how trimodal fusion resolves ambiguity. In sarcastic turns (e.g., \textit{``Oh, that is just brilliant!''}), lexical representations predict \textit{Joy}, but acoustic pitch variations correctly guide Trimodal DGCA to predict \textit{Anger}. Similarly, acoustic tremor distinguishes \textit{Fear} from \textit{Surprise}, and visual cues disambiguate \textit{Disgust}.

\begin{table*}[t]
\centering
\small
\caption{Qualitative conversational case studies demonstrating multimodal disambiguation on MELD.}
\label{tab:case_studies}
\begin{tabular}{p{5.2cm}ccccp{4.2cm}}
\toprule
\textbf{Utterance Transcript} & \textbf{Ground Truth} & \textbf{Text-Only} & \textbf{Trimodal DGCA} & \textbf{Gating ($\alpha_t/\alpha_a/\alpha_v$)} & \textbf{Affective Rationale} \\
\midrule
``Oh, that is just brilliant!'' & Anger & Joy & Anger & 0.22 / 0.52 / 0.26 & Sarcastic utterance disambiguated by tense acoustic pitch and prosody. \\
``What was that loud noise?!'' & Fear & Surprise & Fear & 0.18 / 0.49 / 0.33 & Audio tremolo and dilated facial cues flip prediction from Surprise to Fear. \\
``I cannot eat this food.'' & Disgust & Neutral & Disgust & 0.20 / 0.28 / 0.52 & Facial grimace and lip curl visual representations resolve disgust. \\
``Yes, I will be attending.'' & Neutral & Neutral & Neutral & 0.68 / 0.18 / 0.14 & Factual turn where lexical content dominates over neutral tone. \\
\bottomrule
\end{tabular}
\end{table*}
